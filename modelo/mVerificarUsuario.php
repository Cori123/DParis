<?php 
	...
	if (isset($_POST['ingresar'])) {
		...
		$proc_ver_user = "call verificar_persona_usuario('$usuario')";
		...
		if (mysqli_num_rows($bd_verificar_usuario)<=0) {
			...
			if ($contraencrypt==$usr_contrasenia) {
				...
				if ($usr_estado == 'Permitido') {
					...
					$nombre = $consulta_usuario['usr_nombre'];
					$apellido_paterno = $consulta_usuario['usr_apellido_paterno'];
					$apellido_materno = $consulta_usuario['usr_apellido_materno'];
					$tipo = $consulta_usuario['usr_tipo'];

				  	$_SESSION['usuario_nombre_completo'] = $nombre." ".$apellido_paterno;
				  	$_SESSION['usuario_dni'] = $usuario;
					$_SESSION['usuario_tipo'] = $tipo;
					$_SESSION['usuario_estado'] = $usr_estado;
